...
appliance_work_dir="/"
ove_work_dir="$appliance_work_dir/ove"
...
function extract_live_iso() {
    ...
    mkdir -p "${ove_work_dir}"
    echo "Copying extracted appliance ISO contents to a writable directory."
    $SUDO rsync -aH --info=progress2 "${appliance_mnt_dir}/" "${ove_work_dir}/"
    $SUDO chown -R $(whoami):$(whoami) "${ove_work_dir}/"
...
function setup_agent_artifacts() {
    ...
    local image_dir="${ove_work_dir}"/images/"${image}"
...
function create_ove_iso() {
    if [ ! -f "${agent_ove_iso}" ]; then
        local boot_image=$ove_work_dir/images/efiboot.img
